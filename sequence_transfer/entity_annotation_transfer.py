...
class EntityAnnotationBase:
    """
    An entity is a sequence (without holes) of signs. A token is a sign, a letter is a sign. A sequence of two tokens
     could be an entity
    An annotation is a name in some namespace
...
class EntityAnnotationSequence(ContextualizedSequence):
    ...
    def new(annotations: List[str], annotation_sequence_format: str = BILUO):
        if annotation_sequence_format == EntityAnnotationSequence.BILUO:
            return EntityAnnotationSequence(0, len(annotations),
                                            AnnotationSequenceContext(
                                                biluo_annotations_to_entity_annotations(annotations)))
    ...
    def convert(self, annotation_sequence_format):
        if annotation_sequence_format == EntityAnnotationSequence.BILUO:
            return entity_annotations_to_biluo_annotations(self._context.context)
        return
...
def handle_undefined(annotations: Iterator[EntityAnnotation]) -> List[EntityAnnotation]:
    ...
    new_annotations = []
    undefined = 0
    for i, (signature, group) in enumerate(groupby(annotations, key=lambda x: x.signature)):
...
class EntityAnnotationTransfer:
    def __init__(self, magic_transfer: MagicTransfer):
        self._magic_transfer = magic_transfer

    def apply(self, annotation_sequence: EntityAnnotationSequence) -> EntityAnnotationSequence:
        if annotation_sequence.size != self._magic_transfer.source.size:
            raise ValueError("Annotation sequence size != transfer source size")  # TODO custom exception

        source = self._magic_transfer.source
        target = self._magic_transfer.target
        inverse_transfer = self._magic_transfer.invert()

        transferred_annotations = map(handle_conflicts,
        ...
        z = handle_undefined(transferred_annotations)
